...
<div class="p-6 md:p-8 space-y-6 min-h-screen font-sans text-text">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-extrabold tracking-tight text-primary">Edit Produk</h1>
            <p class="text-sm text-muted mt-0.5">Memperbarui data atribut produk: <span class="font-semibold text-text">{{ $product->code }}</span></p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-border/40 p-6 md:p-8 shadow-sm">
        <form action="{{ route(auth()->user()->role . '.product.update', $product->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    <label for="stock" class="text-sm font-semibold text-text">Stok <span class="text-danger">*</span></label>
            ...
            <div class="space-y-2">
                <label for="description" class="text-sm font-semibold text-text">Deskripsi Produk</label>
                <textarea name="description" id="description" rows="4"
                          class="w-full px-4 py-3 rounded-xl border border-border/60 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all text-sm bg-bg text-text resize-y @error('description') border-danger/50 focus:ring-danger/20 focus:border-danger @enderror">{{ old('description', $product->description) }}</textarea>
