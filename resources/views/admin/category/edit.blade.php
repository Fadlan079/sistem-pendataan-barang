...
<div class="p-4 md:p-6 lg:p-8 space-y-6 min-h-screen font-sans text-text">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl md:text-2xl font-extrabold tracking-tight text-primary">Edit Kategori</h1>
            <p class="text-xs md:text-sm text-muted mt-0.5">Memperbarui informasi untuk kategori sistem.</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-border/40 p-6 md:p-8 shadow-sm max-w-3xl">
        <form action="{{ route(auth()->user()->role . '.category.update', $category->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                <label for="description" class="text-sm font-semibold text-text">Deskripsi</label>
                <textarea name="description" id="description" rows="4"
                          class="w-full px-4 py-3 rounded-xl border border-border/60 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all text-sm bg-bg text-text resize-y @error('description') border-danger/50 focus:ring-danger/20 focus:border-danger @enderror">{{ old('description', $category->description) }}</textarea>
